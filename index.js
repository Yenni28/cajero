...
const validatePin = (pin_keys) => {
  if (pin_keys.toLocaleString().replaceAll(",", "") === "0000") {
    ...
    screen.innerText = "Pin ingresado es correcto ahora presiona la tecla verde";
    state = 'valido';
  } else {
    console.log(pin_keys)
...
keys.addEventListener("click", (event) => {
  ...
  if ((parseInt(id) || id === '0') && typeof parseInt(id) === 'number') {
    // Sí tiene id va a entrar
    // Recogemos
     //el valor de el id y hacemos push al array pin
    ...
    if (state === 'login') {
      pin = [];
      screen.innerText = "Operacion cancelada";
    }

  } else if (id === 'btn-success') {
    console.log('btn-success');
    if (state === 'valido') {
      screen.innerText = "Operacion en proceso";
    }
    ...
    if (pin.length) {
      pin.pop();
      screen.innerText = "Digito eliminado";
    }
  } else {
